            ...
            <header className="mb-12 flex flex-col xl:flex-row xl:items-end justify-between gap-8 animate-in fade-in slide-in-from-right duration-700">
                <div className="space-y-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                             <span className="text-amber-600 font-black text-[10px] uppercase tracking-[0.4em] block mb-1">Executive Analytics</span>
                             <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tight">Strategic Intelligence</h1>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <div className="bg-white border border-slate-100 p-2 rounded-2xl flex items-center gap-2 shadow-sm">
                        <button className="px-4 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest">Q3 2026</button>
                        <button className="px-4 py-2 text-slate-400 text-[10px] font-black uppercase tracking-widest hover:text-slate-900 transition-colors">Historical</button>
                    </div>
                    <button className="px-8 py-4 bg-amber-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-3 shadow-xl shadow-amber-200 hover:bg-amber-600 transition-all">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 bg-white rounded-[3rem] border border-slate-100 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.03)] p-10 relative overflow-hidden group">
                    ...
                    <div className="aspect-[16/7] bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center p-12 text-center group-hover:bg-white transition-all duration-700 relative z-10">
                        ...
                        <p className="text-slate-400 font-bold max-w-sm italic leading-relaxed">
                ...
                <div className="lg:col-span-4 bg-slate-900 rounded-[3rem] p-10 text-white shadow-2xl flex flex-col justify-between group overflow-hidden relative">
                    <div className="relative z-10">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-500/20 text-emerald-400 rounded-full text-[9px] font-black uppercase tracking-widest mb-8 border border-emerald-500/20">
                             Board Briefing Ready
                        </div>
                        <h3 className="text-4xl font-black tracking-tighter leading-[0.95] mb-6">Green Campus <br /> Initiative</h3>
                        <p className="text-slate-400 font-medium leading-relaxed mb-10">
                            Smart sector S-04 achieved a record energy recovery rate of 34% this month. Ready for executive review.
                        </p>
                    </div>

                    <button className="bg-white text-slate-900 py-5 rounded-2xl font-black text-sm shadow-xl shadow-white/5 hover:bg-slate-100 transition-all active:scale-95 flex items-center justify-center gap-3 relative z-10">
...
const KPICard = ({ title, value, trend, icon, desc, accent }) => (
    <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100 hover:shadow-2xl hover:shadow-slate-200 transition-all duration-500 hover:-translate-y-2 group flex flex-col h-full">
        <div className="flex items-center justify-between mb-8">
            ...
            <div className={`text-[10px] font-black px-3 py-1.5 rounded-full ${trend.includes('+') ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                {trend}
            </div>
        </div>
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">{title}</h3>
        <div className="text-5xl font-black text-slate-900 tracking-tighter leading-none mb-6">{value}</div>
