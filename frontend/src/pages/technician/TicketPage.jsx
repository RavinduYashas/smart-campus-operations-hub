            ...
            <header className="mb-12 flex flex-col lg:flex-row lg:items-center justify-between gap-8 animate-in fade-in slide-in-from-left duration-700">
                <div className="space-y-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                             <span className="text-blue-600 font-bold text-[10px] uppercase tracking-[0.4em] block mb-1">Queue Management</span>
                             <h1 className="text-4xl md:text-6xl font-bold text-slate-900 tracking-tight">Active Workloads</h1>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <div className="relative group flex-grow lg:flex-none lg:w-80">
                         ...
                         <input 
                            ...
                            className="w-full bg-white border border-slate-100 rounded-2xl py-3.5 pl-11 pr-4 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-4 focus:ring-blue-50 focus:border-blue-200 transition-all shadow-sm"
                         />
                    </div>
                    ...
                    <button className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-xs uppercase tracking-widest flex items-center gap-3 shadow-xl shadow-slate-200 hover:bg-black transition-all">
...
const StatusSummary = ({ icon, label, count, color, bg }) => (
    <div className={`${bg} p-6 rounded-[2rem] border border-white/50 flex flex-col gap-2 group transition-all hover:shadow-lg`}>
        ...
        <div className="flex items-end justify-between mt-2">
            <span className={`text-[10px] font-bold uppercase tracking-widest ${color} opacity-60`}>{label}</span>
    ...
    return (
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 hover:-translate-y-2 flex flex-col group relative overflow-hidden">
            <div className="flex justify-between items-start mb-6">
                <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em]">{id}</span>
                <div className={`px-2.5 py-1 rounded-lg text-[9px] font-bold border uppercase tracking-wider ${priorityColors[priority] || "bg-slate-50 text-slate-400"}`}>
                    {priority}
                </div>
            </div>

            <div className="mb-8">
                <h3 className="text-xl font-bold text-slate-900 leading-tight mb-2 group-hover:text-blue-600 transition-colors">{title}</h3>
